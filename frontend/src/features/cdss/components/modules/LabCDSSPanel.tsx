...
const LAB_PRIORITY_TYPES = new Set([
  "panic_value",
  "critical_result",
  "delta_check",
  "abnormal_result",
...
export function LabCDSSPanel({
  patientId,
  focusedTests = [],
  clinicianName = "Current Laboratory Clinician",
  clinicianRole = "Laboratory Staff",
  className,
}: LabCDSSPanelProps) {
  const cdss = useCDSSModule({ module: "lab", patientId });
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [evidenceRec, setEvidenceRec] = useState<CDSSRecommendation | null>(null);

  const focusTerms = focusedTests.map((item) => item.toLowerCase());
  const matchesFocus = (rec: CDSSRecommendation) => (
    focusTerms.length === 0 ||
    [rec.title, rec.summary, rec.triggeredBy]
      .filter((value): value is string => Boolean(value))
      .some((value) => focusTerms.some((term) => value.toLowerCase().includes(term)))
  );

  const primaryRecs = cdss.activeRecs.filter((rec) => LAB_PRIORITY_TYPES.has(rec.type) && matchesFocus(rec));
  const otherRecs = cdss.activeRecs.filter((rec) => !(LAB_PRIORITY_TYPES.has(rec.type) && matchesFocus(rec)));
    ...
    <div className={cn("rounded-xl border border-border/50 overflow-hidden", className)}>
      <div className="flex items-center justify-between bg-muted/20 px-4 py-2.5 border-b border-border/40">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Lab CDSS</span>
          ...
          {primaryRecs.length > 0 && (
            <>
              ...
              <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide px-1">
                {focusedTests.length > 0 ? "Focused Result Alerts" : "Critical Result Review"}
              </p>
            </>
          )}
